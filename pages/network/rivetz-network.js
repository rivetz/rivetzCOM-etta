...
class Network extends React.Component {
  ...
  render () {
    const { t } = this.props
    const networkSection = 'rivetz-network'
            ...
            <div className='uk-width-1-1@xs uk-width-4-5@l'>
              <div className='section-content'>
                ...
                <div className='section-head' >
                  <h1>{t('rivetz-network.header')}</h1>
                </div>
                <div className='section-details'>
                  <p>{t('rivetz-network.details')}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className='uk-section-large page-section section-content'>
          <div className='uk-container uk-container-large uk-grid-collapse' data-uk-grid>
            <div id='current' className='uk-width-1-1@xs uk-width-3-5@l'>
              <p>{t('rivetz-network.content')}</p>
              <p>{t('rivetz-network.content2')}</p>
              <p>{t('rivetz-network.content3')}</p>
              <p>{t('rivetz-network.content4')}</p>
              <h4>{t('rivetz-network.summary')}</h4>
            </div>
            <div className='section-buttons uk-width-1-1@xs uk-width-3-5@l' data-uk-scrollspy='cls: uk-animation-slide-bottom; repeat: true'>
              <PartnerButton />

              <Link>
                <a className='off-color' href='/network/service-providers'>
                  {t('common:next') + ': ' + t('service-providers.header')}
                </a>
              </Link>
            </div>
          </div>

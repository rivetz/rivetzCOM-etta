...
class Network extends React.Component {
  ...
  render () {
    const { t } = this.props
    const currentURL = '/network/collections'
    const networkSection = 'collections'
    return (
      <LayoutGeneric>
        ...
        <div className='uk-section-large page-section section-content'>
          <div className='uk-container uk-container-large uk-grid-collapse' data-uk-grid>
            <div id='current' className='uk-width-1-1@xs uk-width-3-5@l'>
              <p>{t('collections.content')}</p>
              <p>{t('collections.content2')}</p>
              <p>{t('collections.content3')}</p>
              <p>{t('collections.content4')}</p>
              <h4>{t('collections.summary')}</h4>
            </div>
            <div className='section-buttons uk-width-1-1@xs uk-width-3-5@l' data-uk-scrollspy='cls: uk-animation-slide-bottom; repeat: true'>
              <PartnerButton />

              <Link>
                <a className='off-color' href='/network/dual-roots-of-trust'>
                  {t('common:next') + ': ' + t('drt.header')}
                </a>
              </Link>
            </div>
          </div>
